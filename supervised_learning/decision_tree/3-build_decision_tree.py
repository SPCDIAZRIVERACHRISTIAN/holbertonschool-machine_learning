...
class Node:
    ...
    def get_leaves_below(self):
        '''Finds the leaves below the node

        Returns:
            List: list of leaves with its values
        '''
        # to store the data in list form
        all_leaves = []
        # verify if its left or rigth child
        # and add findings to the all leaves var
        if self.left_child:
            all_leaves.extend(self.left_child.get_leaves_below())
        if self.right_child:
            all_leaves.extend(self.right_child.get_leaves_below())
        return all_leaves
