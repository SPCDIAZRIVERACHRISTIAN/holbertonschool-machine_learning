...
class Decision_Tree():
    ...
    def get_leaf_child(self, node, sub_population):
        '''This method creates leaf of node

        Args:
            node : node to be transformed
            sub_population : the sub population chosen

        Returns:
            instance : returns leaf created
        '''
        # set the target to be the subpopulation
        sub_targets = self.target[sub_population]
        # value assigned using bincount which
        # counts the number of ocurrences
        value = np.bincount(sub_targets).argmax()
        # create a leaf with the value
        leaf_child = Leaf(value)
        # add depth
        leaf_child.depth = node.depth + 1
        # assign the subpopulation
    ...
    def get_node_child(self, node, sub_population):
        ...
        Args:
            node : node to be used for creation
            sub_population : subpopulation of given node

        Returns:
            instance: returns a new node
        '''
        # create a node
        n = Node()
        # add depth
    ...
    def possible_thresholds(self, node, feature):
        '''calculates the possible threshold of feauture'''
        values = np.unique((self.explanatory[:, feature])[node.sub_population])
        return (values[1:] + values[:-1]) / 2

    def Gini_split_criterion_one_feature(self, node, feature):
        '''use gini impurity for only one feature

        Args:
            node (obj)): instance of node
            feature (list): feature of the node to be used

        Returns:
            lists: returns the best threshold and gini weight
        '''
        sub_pop = node.sub_population
        x_sub = self.explanatory[sub_pop, feature]
        y_sub = self.target[sub_pop]
        n = x_sub.shape[0]

        unique_values = np.unique(x_sub)
        ...
        x_sub_2d = x_sub[:, None]
        thresh_1d = thresholds[None, :]

        left_mask = x_sub_2d > thresh_1d

        y_sub_2d = y_sub[:, None]
        classes_1d = classes[None, :]

        class_mask = (y_sub_2d == classes_1d)

        left_f = (left_mask[..., None]) & (class_mask[:, None, :])

        left_count = left_f.sum(axis=0)

        total_left = left_count.sum(axis=1)

        left_probs = np.divide(left_count, total_left[:, None],
                               out=np.zeros_like(left_count, dtype=float),
                               where=(total_left[:, None] != 0))
        gini_left = 1.0 - np.sum(left_probs**2, axis=1)

        total_class_count = class_mask.sum(axis=0)

        right_count = total_class_count[None, :] - left_count
        total_right = right_count.sum(axis=1)

        right_probs = np.divide(right_count, total_right[:, None],
                                out=np.zeros_like(right_count, dtype=float),
                                where=(total_right[:, None] != 0))
        gini_right = 1.0 - np.sum(right_probs**2, axis=1)

        weightedgini = ((total_left / n) * gini_left) + (
            (total_right / n) * gini_right)

        idx_best = np.argmin(weightedgini)
        best_threshold = thresholds[idx_best]
        best_gini = weightedgini[idx_best]

        return best_threshold, best_gini

    def Gini_split_criterion(self, node):
        '''splits criterions on every feature of a node'''
