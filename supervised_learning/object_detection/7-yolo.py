...
class Yolo:
    ...
    def preprocess_images(self, images):
        ...
        Preprocesses images for the YOLO model:
          1) Resizes each image to the input size of
            the model using inter-cubic interpolation
          2) Rescales pixel values to [0, 1]
        ...
        Returns:
            pimages (numpy.ndarray): shape (ni, input_h, input_w, 3)
              where ni is the number of images
            image_shapes (numpy.ndarray): shape (ni, 2) containing the
        ...
        for img in images:
            ...
            image_shapes_list.append((orig_h, orig_w))
    ...
    def show_boxes(self, image, boxes,
        ...
        Args:
            image: np.ndarray with shape (H, W, 3) – unprocessed original image
            boxes: np.ndarray with shape (N, 4) – each row is [x1, y1, x2, y2]
            box_classes: np.ndarray of shape (N,) – class indices for each box
            box_scores: np.ndarray of shape (N,) – scores for each box
            file_name: str – file path where the original image is stored

        Behavior:
            - Draws boxes in blue, thickness = 2
            - Draws class name & score in red, above the box, in size=0.5
            - Waits for a key press:
                - If 's' is pressed, saves the image to ./detections/<file_name>
                - Otherwise, closes the image window without saving
        """
        ...
        for box, cls, score in zip(boxes, box_classes, box_scores):
            ...
            cv2.rectangle(drawn_image, (x1, y1), (x2, y2),
                          (255, 0, 0), 2)
        ...
        cv2.imshow(file_name, drawn_image)
    ...
    def predict(self, folder_path):
        """
        Displays all images in the specified folder along with
        detected objects.

        This function processes each image in the given folder,
        predicts the objects present using
        the YOLO model, and displays the images with bounding
        boxes around the detected objects.
        Each bounding box is accompanied by the object class
        name and the confidence score.
        ...
        - tuple: A tuple containing two elements:
            - predictions: A list of tuples, each containing the
            bounding boxes, class indices,
            and confidence scores for the objects detected in each image.
            - image_paths: A list of paths to the processed images.

        The function performs the following steps for each image:
        1. Loads and preprocesses the images to match the input size
        expected by the YOLO model.
        2. Predicts the objects in the images using the YOLO model.
        3. Processes the model's output to extract bounding box
        coordinates, confidence scores,
        and class indices for each detected object.
        4. Applies non-maximum suppression to filter out overlapping boxes.
        5. Displays the images with annotated bounding boxes and saves
        them if 's' is pressed.
        """
